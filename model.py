...
class Model(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.rnn_enc = LSTMEncoder(config.hidden*2, config.hidden, 1, False, True, 1-config.keep_prob, False)
        ...
        #                           attention=True)
        self.embed_size = 256
        self.decoder = nn.LSTMCell(config.hidden*2,
                                   config.hidden)
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens,question_lens, start_mapping, end_mapping, all_mapping):
        ...
        enc_hiddens,dec_init =  self.rnn_enc(context_output,context_lens)  # start with greedy decoding

        enc_masks = self.generate_sent_masks(enc_hiddens, context_lens)
        ...
        # ques_output = ques_output.transpose_(0, 1)
        # ques_output_casted = ques_output.to(torch.int64)

        # Compute log probability of generating true target words
        # print('P',P.size())
        # print('output', ques_output)
        # import pickle
        # pickle.dump(ques_idxs, open("ques_idx.p", "wb"))
        # # print('casted',ques_output_casted)
        ques_mask_ = ques_mask.transpose_(0, 1)
        ques_mask_ = ques_mask_.unsqueeze(-1)

        target_gold_words_log_prob = torch.gather(
            P, index=ques_idxs.transpose_(0,1).unsqueeze(-1).to(torch.int64)[1:],
            dim=-1) * ques_mask_[1:]
